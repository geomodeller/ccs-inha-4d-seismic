...
class Sim_CCS():
    ...
    """
    version = '0.1.0'
    CMG =  '"C:\\Program Files\\CMG\\GEM\\2021.10\\Win_x64\\EXE\\gm202110.exe"'
    ...
    def run_simulation(self, run_dir = 'test_run', input_parameters = {}, bhp_constraint = None):
        ...
        for key, val in input_parameters.items():
            if key.startswith('porosity'):
                por_params_values = val
            if key.startswith('permeability'):
                perm_params_values = val
        
        # copy data file with replacing well contraints:
        shutil.copyfile(self.CMG_GEM_data_file, data_file_full_dir)
        
        # create schedules:
        self._make_new_porosity(por_params_values, run_dir=run_dir)
        self._make_new_permeability(perm_params_values, run_dir=run_dir)
    ...
    def ensemble_simulation(self, sub_dir = 'test', input_parameters = {}, bhp_constraint = None):
        out_file =  self.CMG_GEM_data_file.split('\\')[-1].replace('.dat','.out')
        ...
        for key, val in input_parameters.items():
            ...
            if key.startswith('Facies'):
                facies_params_values = val    
            
        
        self.N_ensemble = por_params_values.shape[0]
        
        for i in range(self.N_ensemble):
            run_dir = f'{sub_dir}_{i}'
            ...
            self._make_new_porosity(por_params_values[i], run_dir=run_dir)
            self._make_new_permeability(perm_params_values[i], run_dir=run_dir)
            self._make_new_Facies(facies_params_values[i], run_dir=run_dir)

    def _run_ccs(self, run_dir):
        os.chdir(run_dir)
        os.system("{} -f {}".format(self.CMG, self.CMG_GEM_data_file.split('\\')[-1]))
        os.chdir('..')
    
    def run_multiple_CMG(self, N_thread, sub_dir = 'Test'):
        out_file =  self.CMG_GEM_data_file.split('\\')[-1].replace('.dat','.out')
        run_dir = []
        for i in range(self.N_ensemble):
            if os.path.isdir(f'{sub_dir}_{i}') and os.path.isfile(os.path.join(f'{sub_dir}_{i}', out_file)):
                print(f'{sub_dir}_{i} simulation result is already there ... please check your run_directory')
            else:
                run_dir.append(f'{sub_dir}_{i}')
        
        pool = Pool(processes = N_thread)
        
        pool.map(func = self._run_ccs, iterable = run_dir)
    ...
    def _make_new_porosity(self, por = -np.ones((NZ, NY, NX)), run_dir = '',):
        f = open(os.path.join(run_dir, 'porosity.inc'), 'w')
        f.write('*POR *ALL \n')
        for k in range(self.NZ):
            for j in range(self.NY):
                for i in range(self.NX):
                    f.write(str(por[k, j, i]) + '\n')
        f.close()
    
    def _make_new_permeability(self, perm = -np.ones((NZ, NY, NX)), run_dir = '',):
        f = open(os.path.join(run_dir, 'permeability.inc'), 'w')
        f.write('*PERMI *ALL \n')
        for k in range(self.NZ):
            for j in range(self.NY):
                for i in range(self.NX):
                    f.write(str(perm[k, j, i]) + '\n')
        f.close()
        
    def _make_new_Facies(self, Facies = -np.ones((NZ, NY, NX)), run_dir = '',):
        f = open(os.path.join(run_dir, 'rocktype.inc'), 'w')
        f.write('*RTYPE *ALL \n')
        for k in range(self.NZ):
            for j in range(self.NY):
                for i in range(self.NX):
                    f.write(str(int(Facies[k, j, i])+2) + '\n')
        f.close()
    ...
    def read_top_depth(self, run_dir = ''):
        nx, ny = self.NX, self.NY
        f = open(os.path.join(run_dir, 'zcorn.inc'), mode='r')
        lines = []
        for line in f:
            if line.split(' ')[0] == '*ZCORN':
                continue
            lines.append(np.float32(line.split(' ')[0]))
        f.close
        NW_T = np.zeros((ny,nx))
        NE_T = np.zeros((ny,nx))
        SW_T = np.zeros((ny,nx))
        ...
        for j in range(ny):
            for i in range(nx):
                NW_T[j, i] = lines[2*i+4*ny*j]
                NE_T[j, i] = lines[2*i+4*ny*j+1]
                SW_T[j, i] = lines[2*i+4*ny*j+2*ny]
                SE_T[j, i] = lines[2*i+4*ny*j+2*ny+1]

        TOP_D = (NW_T + NE_T + SW_T + SE_T)/4
        TOP_D = TOP_D[::-1]
